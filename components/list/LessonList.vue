<template>
  <div class="block">
    ...
    <delete-popup v-if="deletePopup.show" title="Удаление предмета" @delete="deleteLesson" @close="deletePopup.show = false">
      Вы уверены, что хотите удалить "{{ deletePopup.active.title }}"
    </delete-popup>
    <update-lesson-popup v-if="updatePopup.show" :lesson="updatePopup.active" @close="updatePopup.show = false" />
    <add-lesson-popup v-if="showAddPopup" :course="course" @add="addLesson" @close="showAddPopup = false" />
...
import AddLessonPopup from '~/components/popup/add-popup/AddLessonPopup.vue'
import UpdateLessonPopup from '~/components/popup/update-popup/UpdateLessonPopup.vue'
import ContextPopup from '~/components/ContextPopup.vue'

@Component({
  components: { ContextPopup, UpdateLessonPopup, AddLessonPopup, DeletePopup }
})
export default class extends Vue {
  ...
  deleteLesson () {
    this.$axios.$delete('/api/lesson/' + this.deletePopup.active.id).then(() => {
