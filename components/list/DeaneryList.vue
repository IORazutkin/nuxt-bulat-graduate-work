<template>
  <div class="block">
    ...
    <delete-popup v-if="deletePopup.show" title="Удаление работника" @delete="deleteDeanery" @close="deletePopup.show = false">
      Вы уверены, что хотите удалить "{{ deletePopup.active.fullName }}"
    </delete-popup>
    <update-deanery-popup v-if="updatePopup.show" :user="updatePopup.active" @close="updatePopup.show = false" />
    <add-deanery-popup v-if="showAddPopup" :institute="institute" @add="addDeanery" @close="showAddPopup = false" />
...
import AddDeaneryPopup from '~/components/popup/add-popup/AddDeaneryPopup.vue'
import { Institute } from '~/types/institute/institute'
import DeletePopup from '~/components/popup/DeletePopup.vue'
import UpdateDeaneryPopup from '~/components/popup/update-popup/UpdateDeaneryPopup.vue'
...
@Component({
  components: { ContextPopup, UpdateDeaneryPopup, DeletePopup, AddDeaneryPopup }
})
