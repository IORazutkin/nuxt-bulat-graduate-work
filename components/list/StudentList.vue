<template>
  <div class="block">
    ...
    <delete-popup v-if="deletePopup.show" title="Удаление студента" @delete="deleteStudent" @close="deletePopup.show = false">
      Вы уверены, что хотите удалить "{{ deletePopup.active.fullName }}"
    </delete-popup>
    <update-student-popup v-if="updatePopup.show" :user="updatePopup.active" @close="updatePopup.show = false" />
    <add-student-popup v-if="showAddPopup" :institute="institute" @add="addStudent" @close="showAddPopup = false" />
...
import AddStudentPopup from '~/components/popup/add-popup/AddStudentPopup.vue'
import DeletePopup from '~/components/popup/DeletePopup.vue'
import UpdateStudentPopup from '~/components/popup/update-popup/UpdateStudentPopup.vue'
...
@Component({
  components: { ContextPopup, UpdateStudentPopup, DeletePopup, AddStudentPopup }
})
