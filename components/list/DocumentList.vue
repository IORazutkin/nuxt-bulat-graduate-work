<template>
  <div class="block">
        ...
        >
          ...
          <div class="item__actions">
            <template v-if="item.task">
              <button class="item__btn agree" @click.stop="verifyDocument(item, true)">
                &nbsp;
              </button>
              <button class="item__btn disagree" @click.stop="verifyDocument(item, false)">
                &nbsp;
              </button>
            </template>
            <template v-else>
    ...
    <delete-popup v-if="deletePopup.show" title="Удаление документа" @delete="deleteDocument" @close="deletePopup.show = false">
      Вы уверены, что хотите удалить "{{ deletePopup.active.title }}"
    </delete-popup>
    <update-full-document-popup v-if="updatePopup.show" :document="updatePopup.active" @close="updatePopup.show = false" />
    <add-document-popup v-if="showAddPopup" @add="addDocument" @close="showAddPopup = false" />
    <context-popup v-if="mobileActive" @close="mobileActive = null">
      <template v-if="!mobileActive.task">
      ...
      <template v-else>
        <li @click="verifyDocument(mobileActive, true)">
          Подтвердить
        </li>
        <li class="delete" @click="verifyDocument(mobileActive, false)">
          Отклонить
        </li>
      </template>
    </context-popup>
...
export default class extends Vue {
  ...
  get filteredList () {
    return this.list.filter(item => item.title.toLowerCase().includes(this.filter.toLowerCase()))
  }
  ...
  deleteDocument () {
    this.$axios.$delete('/api/document/' + this.deletePopup.active.id).then(() => {
  ...
  verifyDocument (document: Document, isVerify: boolean) {
    this.$axios.$get('/api/document/verify/' + document.id, {
      params: {
        status: isVerify
      }
    }).then(() => {
      const index = this.list.indexOf(document)
      this.list.splice(index, 1)
    })
  }
}
...
.block {
  ...
  &__list {
    ...
    &__item {
      ...
      cursor: pointer;
      background: white url(~@/assets/img/icons/link.svg) no-repeat right 15px center/12px;
