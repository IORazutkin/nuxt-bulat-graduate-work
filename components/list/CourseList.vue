<template>
  <div class="block">
    ...
    <div class="block__list">
      <template v-if="filteredList.length > 0">
        <div
          ...
          @click="$router.push({ path: item.id.toString(), append: true })"
        >
          {{ item.title }}
    ...
    <delete-popup v-if="deletePopup.show" title="Удаление направления" @delete="deleteCourse" @close="deletePopup.show = false">
      Вы уверены, что хотите удалить "{{ deletePopup.active.title }}"
    </delete-popup>
    <update-course-popup v-if="updatePopup.show" :course="updatePopup.active" @close="updatePopup.show = false" />
    <add-course-popup v-if="showAddPopup" :institute="institute" @add="addCourse" @close="showAddPopup = false" />
...
import AddCoursePopup from '~/components/popup/add-popup/AddCoursePopup.vue'
import { Institute } from '~/types/institute/institute'
import DeletePopup from '~/components/popup/DeletePopup.vue'
import UpdateCoursePopup from '~/components/popup/update-popup/UpdateCoursePopup.vue'
import ContextPopup from '~/components/ContextPopup.vue'

@Component({
  components: { ContextPopup, UpdateCoursePopup, DeletePopup, AddCoursePopup }
})
export default class extends Vue {
  ...
  deleteCourse () {
    this.$axios.$delete('/api/course/' + this.deletePopup.active.id).then(() => {
...
.block {
  ...
  &__list {
    ...
    &__item {
      ...
      cursor: pointer;
      background: white url(~@/assets/img/icons/right-arrow.svg) no-repeat right 15px center/12px;
