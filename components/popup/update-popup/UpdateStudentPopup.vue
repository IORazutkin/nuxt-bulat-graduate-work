<template>
  <popup title="Изменение студента" @close="$emit('close')">
...
export default class extends Vue {
  ...
  async fetch () {
    await this.$axios.$get('/api/course/institute/' + this.user.group?.course.institute.id).then((lessons) => {
      this.courseList.push(...(lessons || []))
    })

    await this.$axios.$get('/api/group/course/' + this.user.group?.course.id).then((groups) => {
  ...
  updateStudent () {
    ...
    this.$axios.$patch('/api/student/' + this.user.id, this.formData).then((user: User) => {
