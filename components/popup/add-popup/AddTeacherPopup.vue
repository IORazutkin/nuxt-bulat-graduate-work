<template>
  <popup title="Добавление преподавателя" @close="$emit('close')">
...
export default class extends Vue {
  ...
  async fetch () {
    await this.$axios.$get('/api/lesson/institute/' + this.institute.id).then((lessons) => {
  ...
  addTeacher () {
    ...
    this.$axios.$post('/api/teacher', this.formData).then((user: User) => {
      this.$emit('add', user)
      this.$emit('close')
    }).catch((err) => {
